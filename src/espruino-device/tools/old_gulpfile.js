...
const distDir = './dist/profiles/train-driver';
const srcDir = './src/profiles/train-driver';
const appFileName = 'train-driver-app.js';
...
gulp.task('espruino-console', ['clear-espurino-watch-file'], (cb) => {
  const buildproc = fork(
    require.resolve('espruino/bin/espruino-cli'),
    ['--board', envConfig.board, '-d', envConfig.device, '-w', espConsoleBeingWatchedFileName],
...
gulp.task('buil-send-to-espruino-console', ['build'], (cb) => {
  const content = fs.readFileSync(espReadyBundlePath);
  fs.writeFile(
    espConsoleBeingWatchedFilePath,
    content,
